...
interface LogInOutButtonsProps {
  onAction: (action: "in" | "out") => void
  loadingAction?: "in" | "out" | null
}

export function LogInOutButtons({ onAction, loadingAction = null }: LogInOutButtonsProps) {
  const isLoginLoading = loadingAction === "in"
  const isLogoutLoading = loadingAction === "out"
  ...
  return (
    <div className="flex flex-col gap-6 w-full items-center">
      {/* Log In Button */}
      <button
        onClick={() => !isAnyLoading && onAction("in")}
      ...
      {/* Log Out Button */}
      <button
        onClick={() => !isAnyLoading && onAction("out")}
        disabled={isAnyLoading}
        className="relative group w-40 h-40 rounded-full flex items-center justify-center transition-all duration-300 hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
      >
        {/* Outer glow ring */}
        <div className="absolute inset-0 rounded-full bg-gradient-to-br from-red-400 to-orange-500 opacity-50 blur-xl group-hover:opacity-70 transition-opacity" />

        {/* Border ring */}
        <div className="absolute inset-0 rounded-full bg-gradient-to-br from-red-400 to-orange-500 p-[3px]">
          <div className="w-full h-full rounded-full bg-[#0a0a0a]" />
        </div>
        ...
        <div className="relative z-10 flex items-center gap-2">
          {isLogoutLoading ? (
            ...
            <>
              <span className="text-xl font-semibold text-white">Log Out</span>
